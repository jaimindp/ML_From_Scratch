...
def compute_distances(Xtrain, X):
	...
	B = np.sum(Xtrain**2, axis = 1).reshape(Xtrain.shape[0],1)
...
def predict_labels(k, ytrain, dists):
	...
	for i in range(dists.shape[0]):
		low_dists = np.argsort(dists[i])
		classes = [ytrain[j] for j in low_dists[:k]]
		counts = sorted([[k,v] for k,v in collections.Counter(classes).items()], key = lambda x : (-x[1], x[0]))
		ypred[i] = counts[0][0]
...
def find_best_k(K, ytrain, dists, yval):
	...
	validation_accuracy = []
	for k in K:
		preds = predict_labels(k, ytrain, dists)
		this_acc = compute_accuracy(yval, preds)
		if this_acc > best_acc:
			best_acc = this_acc
			best_k = k
		validation_accuracy.append(this_acc)
	print(best_k)
