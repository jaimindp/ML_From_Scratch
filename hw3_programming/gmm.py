...
class GMM():
    ...
    def fit(self, x):
        ...
        if (self.init == 'k_means'):
            ...
            print(self.means, self.pi_k, self.variances)
        ...
        #     x_mu = x - self.means[c]
        #     gamma[c] = self.pi_k[c] / (np.sqrt(2*np.pi)**D * np.abs(self.variances[c])) * np.exp(-0.5*np.dot(x_mu.T,x_mu))
        #     tot += gamma[c]
    ...
    class Gaussian_pdf():
        ...
        def getLikelihood(self,x):
            ...
            p = np.exp(-0.5* np.dot((x-self.mean), np.dot(np.inverse(self.variance),(x-self.mean).T))/np.sqrt(self.c))
            # DONOT MODIFY CODE BELOW THIS LINE
            return p
