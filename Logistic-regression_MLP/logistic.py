...
def multinomial_train(X, y, C, 
    ...
    labels = convert_to_onehot(y,C)

    for i in range(max_iterations):

        softmx = np.exp(np.dot(X,w.T) + b) # -> N x C
        proba = np.divide(softmx.T, np.sum(softmx, axis=1)).T
...
def multinomial_predict(X, w, b):
    ...
    preds = np.argmax(np.exp(np.dot(X, w.T) + b), axis=1)
...
def OVR_train(X, y, C, w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    for c in range(C):
        labels = np.asarray(y==c).astype(int)
        w[c], b[c] = binary_train(X,labels,step_size=step_size,max_iterations=max_iterations)
...
def OVR_predict(X, w, b):
    ...
    classes = sigmoid(np.dot(X,w.T) + b)
